...
const Filter: FunctionComponent<FilterProps> = ({params, onSubmit}) => {
	...
	const searchByValue = useMemo(() => {
		...
		if (Object.keys(params).includes("noTelp")) {
			return "noTelp";
		}
		return "";
	}, [params]);
	...
	const onResetSearch = () => {
		const updatedParamsWithoutSearch: SimpedesUmiApplicationCollectionParams = {
			...params,
		};
		...
		if (watchFields.searchBy === "noTelp") {
			updatedParamsWithoutSearch["noTelp"] = "";
		}
	...
	const dropdownMenuList = [
		...
		},
		{
			label: "No Telp",
			onClick: () => {
				setValue("searchBy", "noTelp", {shouldValidate: true});
			},
		},
	];
