...
const statusApprovals = [
	...
	{label: "AUTO-REJECT", value: "AUTO_REJECTED"},
...
const FloatingFilter: FunctionComponent<FloatingFilterProps> = ({
	...
	const isSubmitDisabled = useMemo(() => {
		if (!startDate && !endDate && statusList.length === 0) {
			return true;
		} else {
			return false;
		}
	...
	}, [startDate, endDate, statusList.length]);
	...
	const submitFilter = useCallback(() => {
		...
		handleClose();
	}, [params, startDate, endDate, statusList, onParamsChange, handleClose]);
	...
	const resetFilter = useCallback(() => {
		...
		const resetParams: ManualHistoryApprovalCollectionParams = {
			...params,
			page: 1,
			limit: 10,
			startDate: undefined,
		...
		>
			...
			>
				<div className="container-filter-simpedes-umi">
					...
					<div
						className={"dropdown-filter-contents"
							.concat(isShow ? " active" : "")
							.concat(
								isShowEndDatepicker || isShowStartDatepicker
									? " h-[350px]"
									: "",
							)}
					>
						...
						<div className="flex flex-col gap-2 mb-4">
							<span className="label-filter-by">Status Approval</span>
							<div className="grid grid-cols-2 items-center gap-2">
								{statusApprovals?.map((item, index) => (
									<Checkbox
										key={item.value}
										label={item.label}
										value={item.value}
										data-testid="rejected-checkbox"
										onChange={(e) => {
											onClickStatus(e.target.value);
										}}
										checked={statusList.includes(item.value)}
									/>
								))}
							</div>
						</div>
